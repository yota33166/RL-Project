...
class RLLogger:
    ...
    def log_end_time(self, task_name: str, epoch: Optional[int] = None) -> None:
        ...
        if epoch is not None:
            ...
            self.timing_logger.info(f"Epoch {epoch}: {task_name} took {elapsed_time:.4f} seconds")
    ...
    def log_summery(self) -> None:
        ...
        self.timing_logger.info(f"Total training time: {timedelta(seconds=total_time)} seconds")
        ...
        for task_name, duration in self.durations.items():
            ...
            self.logger.info(f"{task_name}: {timedelta(seconds=duration)} seconds ({percentage:.2f}%)")
            self.timing_logger.info(f"{task_name}: {timedelta(seconds=duration)} seconds ({percentage:.2f}%)")
    ...
    def log_metric(self, metrics: Dict[str, float], epoch: int) -> None:
        ...
        for name, value in metrics.items():
            category = name.split('/')[0] if '/' in name else 'Metrics'
